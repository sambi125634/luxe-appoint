...
const ZONES: ZoneConfig[] = [
  { key: "active", label: "Aktywne", emoji: "💚", color: "text-emerald-600", bgGradient: "from-emerald-500/20 to-emerald-500/5", borderPulse: false, avatarBg: "bg-emerald-100 text-emerald-700", progressColor: "bg-emerald-500" },
  { key: "attention", label: "Uwaga", emoji: "🟡", color: "text-amber-600", bgGradient: "from-amber-500/20 to-amber-500/5", borderPulse: false, avatarBg: "bg-amber-100 text-amber-700", progressColor: "bg-amber-500" },
  { key: "risk", label: "Ryzyko", emoji: "🟠", color: "text-orange-600", bgGradient: "from-orange-500/20 to-orange-500/5", borderPulse: true, avatarBg: "bg-orange-100 text-orange-700", progressColor: "bg-orange-500" },
  { key: "lost", label: "Utracone", emoji: "🔴", color: "text-rose-600", bgGradient: "from-rose-500/20 to-rose-500/5", borderPulse: true, avatarBg: "bg-rose-100 text-rose-700", progressColor: "bg-rose-500" },
];
...
export function RetentionHealthBoard({ clients, compact = false, onClientClick }: RetentionHealthBoardProps) {
  ...
  const renderZoneColumn = (zone: ZoneConfig) => {
    ...
    return (
      <div key={zone.key} className="flex flex-col gap-2">
        ...
        <div className="space-y-2 min-h-[100px]">
          {zoneClients.map(c => (
            <ClientCard key={c.id} client={c} zone={zone} onClientClick={onClientClick} />
          ))}
  ...
  if (compact) {
    // Compact version for dashboard home
    const counts = ZONES.map(z => ({ ...z, count: classified[z.key].length }));
    return (
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base">📊 Retencja klientek</CardTitle>
            <div className="flex items-center gap-2 text-sm text-emerald-600 font-medium">
              <TrendingUp className="w-4 h-4" />
              {retentionRate}% <span className="text-xs text-muted-foreground">+5pp</span>
            </div>
          </div>
        </CardHeader>
        <CardContent>
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-4">
            ...
            <div>
              <CardTitle className="text-lg">Retencja klientek</CardTitle>
              <div className="flex items-center gap-1.5 mt-0.5">
                <TrendingUp className="w-4 h-4 text-emerald-500" />
                <span className="text-sm text-emerald-600 font-medium">+5pp vs poprzedni miesiąc</span>
              </div>
            </div>
          </div>
          <Badge variant="outline" className="text-xs">
            {totalClients} klientek łącznie
          </Badge>
        </div>
      </CardHeader>
      <CardContent>
        {isMobile ? (
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as ZoneKey)}>
            <TabsList className="w-full grid grid-cols-4">
              {ZONES.map(z => (
                <TabsTrigger key={z.key} value={z.key} className="text-xs gap-1">
                  {z.emoji} {classified[z.key].length}
                </TabsTrigger>
              ))}
            </TabsList>
            {ZONES.map(z => (
              <TabsContent key={z.key} value={z.key} className="mt-3">
                {renderZoneColumn(z)}
              </TabsContent>
            ))}
          </Tabs>
          ...
          <div className="grid grid-cols-4 gap-4">
            {ZONES.map(z => renderZoneColumn(z))}
          </div>
        )}
      </CardContent>
    </Card>
