...
export function useTrueProfit() {
  ...
  const hasStaffRates = false; // future: check staff_members.hourly_rate
  ...
  const serviceProfits: ServiceProfit[] = useMemo(() => {
    ...
    return services
      ...
      .map((s) => {
        ...
        const staffCost = (s.duration / 60) * DEFAULT_HOURLY_RATE;
  ...
  }, [services, monthAppointments, getMaterialCost]);
  ...
  const calcSummary = (appointments: typeof monthAppointments): ProfitSummary => {
    ...
    for (const apt of appointments) {
      ...
      if (svc) {
        ...
        staffCosts += (svc.duration / 60) * DEFAULT_HOURLY_RATE;
      }
