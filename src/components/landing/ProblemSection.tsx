    ...
    <section className="landing-section-dark landing-section-spacing relative overflow-hidden">
      <div className="max-w-[1200px] mx-auto px-[max(24px,5vw)] relative z-10">
        ...
        <AnimatedHeadline className="text-center max-w-3xl mx-auto mb-16">
          <p className="eyebrow tracking-widest mb-6" style={{ color: "rgba(245,245,247,0.4)" }}>
            Uwaga: Właścicielki salonów beauty w Polsce
          </p>

          <h2 className="headline-section mb-6" style={{ color: "#f5f5f7" }}>
            Czy pracujesz pełną parą...
            <br />
            <span className="subheadline" style={{ color: "rgba(245,245,247,0.5)", fontFamily: "'Inter', sans-serif" }}>
          ...
          {painCards.map((card, i) => (
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-bold mb-2 text-base" style={{ color: "#f5f5f7", fontFamily: "'Inter', sans-serif" }}>{card.headline}</h3>
                  <p className="text-sm leading-relaxed landing-text-muted-dark">{card.desc}</p>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        >
          <div className="landing-card-dark p-8 md:p-10" style={{ borderColor: "rgba(239,68,68,0.15)" }}>
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                <h3 className="font-bold text-lg" style={{ color: "#f5f5f7", fontFamily: "'Inter', sans-serif" }}>Kalkulator strat</h3>
                <p className="text-sm landing-text-muted-dark">Ile tracisz na no-showach?</p>
              </div>
            </div>
            ...
            <div className="space-y-6 mb-8">
              <div>
                <div className="flex justify-between mb-3">
                  <Label className="text-sm" style={{ color: "rgba(245,245,247,0.7)" }}>Miesięczny przychód salonu (zł)</Label>
                  <span className="font-bold" style={{ color: "#f5f5f7" }}>{monthlyRevenue.toLocaleString("pl-PL")} zł</span>
                </div>
                <Slider value={[monthlyRevenue]} min={2000} max={60000} step={500} onValueChange={([v]) => setMonthlyRevenue(v)} className="[&_[role=slider]]:bg-white" />
              </div>
              <div>
                <div className="flex justify-between mb-3">
                  <Label className="text-sm" style={{ color: "rgba(245,245,247,0.7)" }}>No-showy tygodniowo</Label>
                  <span className="font-bold" style={{ color: "#f5f5f7" }}>{noShows}</span>
                </div>
                <Slider value={[noShows]} min={0} max={15} step={1} onValueChange={([v]) => setNoShows(v)} className="[&_[role=slider]]:bg-white" />
              </div>
              <div>
                <div className="flex justify-between mb-3">
                  <Label className="text-sm" style={{ color: "rgba(245,245,247,0.7)" }}>Średnia wartość wizyty (zł)</Label>
                  <span className="font-bold" style={{ color: "#f5f5f7" }}>{avgVisit} zł</span>
                </div>
                <Slider value={[avgVisit]} min={50} max={600} step={10} onValueChange={([v]) => setAvgVisit(v)} className="[&_[role=slider]]:bg-white" />
              </div>
            </div>

            <div className="rounded-xl p-5 space-y-3 mb-6" style={{ background: "rgba(255,255,255,0.04)" }}>
              <div className="flex justify-between text-sm">
                <span style={{ color: "rgba(245,245,247,0.6)" }}>Straty na no-showach rocznie:</span>
                <span className="font-bold text-red-400">-{noShowLoss.toLocaleString("pl-PL")} zł</span>
              </div>
              <div className="border-t pt-3" style={{ borderColor: "rgba(255,255,255,0.08)" }}>
                <div className="flex justify-between items-baseline">
                  <span className="font-bold" style={{ color: "#f5f5f7" }}>Tracisz łącznie:</span>
                  <motion.span
                    key={totalLoss}
                    initial={{ scale: 0.9 }}
                    animate={{ scale: 1 }}
                    className="font-black text-2xl text-red-400"
                  >
              ...
              <div className="rounded-lg p-3" style={{ background: "rgba(34,197,94,0.08)" }}>
                <div className="flex justify-between">
                  <span className="text-emerald-400 text-sm font-medium">Z Beauty Calendar odzyskasz:</span>
                  <span className="font-bold text-emerald-400">+{totalLoss.toLocaleString("pl-PL")} zł/rok</span>
                </div>
              </div>
            </div>
      ...
      {/* Section transition */}
      <div className="absolute bottom-0 left-0 right-0 h-32 section-fade-to-light pointer-events-none" />
    </section>
  );
};